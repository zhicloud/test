...
class CachedQueue(object):
    ...
    minimum_interval = 5
    minimum_cache = 5
    def __init__(self, index, handler, interval = 20, cache = 50,
                 max_request = 1000, debug = False, logger_name = "CachedQueue"):
        self.index = index
        ...
        if interval < self.minimum_interval:
            self.interval = float(self.minimum_interval)/1000
        else:
            self.interval = float(interval)/1000

        if cache < self.minimum_cache:
            self.cache = self.minimum_cache
        else:
            self.cache = cache
...
##                self.logger.error("<%s>channel %d is full, %d/%d"%(
...
##    request = object()
    ...
    for i in range(max_channel):
        queue = CachedQueue(i, request_handler,
                             debug = True,
                             logger_name = name)
        queue.start()
