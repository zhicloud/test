...
class ChangeMediaTask(BaseTask):
    ...
    def invokeSession(self, session):
        ...
        self.info("[%08X]request change media '%s' into host '%s' to control server '%s'"%
    ...
    def onDeleteSuccess(self, msg, session):
        ...
        self.info("[%08X]change media success, id '%s'"%
    ...
    def onDeleteFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]change media fail, id '%s'"%
                  (session.session_id, session.target))
        self.case_manager.finishTestCase(TestResultEnum.fail)
        session.finish()
        
    def onDeleteTimeout(self, msg, session):
        self.info("[%08X]change media timeout, id '%s'"%
                  (session.session_id, session.target))
