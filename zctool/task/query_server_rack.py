...
class QueryServerRackTask(BaseTask):
    ...
    def invokeSession(self, session):
        ...
        room = param["room"]        
        request.setString(ParamKeyDefine.room, room)
    ...
    def onQuerySuccess(self, msg, session):
        ...
        newstatus = ChangeResuleStatus(status,Status_server_rack)
        ...
        for server_status in server:
            newserver.append(ChangeResuleStatus(server_status,Status_server_rack))
        
        self.info("[%08X]query server rack"%
                       (session.session_id))
    ...
    def onQueryFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X]query server rack fail, id '%s'"%
                  (session.session_id, session.target))
        self.case_manager.finishTestCase(TestResultEnum.fail)
        session.finish()
        
    def onQueryTimeout(self, msg, session):
        self.info("[%08X]query server rack tiemout, id '%s'"%
                  (session.session_id, session.target))
